...
| `SerializableInfinityValue` | Serialization wrapper for exposing `InfinityValue` in the Unity Inspector ([details](./inspector)). |
...
- [Math Helpers](./math) — cost curves, `AffordableCount`, `ToProgress`, `Min`/`Max`/`Clamp`, increment/decrement operators
- [Inspector & Exact Serialization](./inspector) — `SerializableInfinityValue`, `ToExactString`/`TryParseExact`
