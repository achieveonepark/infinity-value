...
| `SerializableInfinityValue` | Unity インスペクターに `InfinityValue` を公開するシリアライズラッパー([詳細](./inspector)) |
...
- [数学ヘルパー](./math) — コストカーブ、`AffordableCount`、`ToProgress`、`Min`/`Max`/`Clamp`、インクリメント/デクリメント演算子
- [インスペクターと厳密なシリアライズ](./inspector) — `SerializableInfinityValue`、`ToExactString`/`TryParseExact`
